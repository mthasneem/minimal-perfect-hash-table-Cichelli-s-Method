...
public static Scanner openTextFile(String fileName)	{
	...
	try{ 
		...
		System.out.println("There have no data in file " + fileName);
	}
	data = null;
	return data;
}
...
public static void printResults(int lineCounter, int wordCounter, Key[] hashTable, int[] keyWordCounter)	{
	...
	for(int i = 0; i < hashTable.length; i += 1)	{
		//System.out.println("   "+((hashTable[i].getPositionInGLast()+hashTable[i].getPositionInGFirst()+hashTable[i].getKeyWord().length())%10)+ hashTable[i].getKeyWord() + ": " + keyWordCounter[i]);
	...
	System.out.println("Total Key Words: " + totalKeys);
}
	
	public static void main(String[] args) {
		//to calculate execution start time
		...
		Scanner keyWordFile = openTextFile("kywrdsOdd.txt");
		...
		//method opens text file then returns a scanner of the file.
		Scanner textFile = openTextFile("tstOdd.txt");
		...
		*/
		...
		//to calculate execution end time
...
}//end of classpublic class Driver
